...
try:
    ...
    def start(message):
        bot.send_message(message.chat.id, '👋🏻 Привет! Это бот для технической поддержки участников.\nЕсли у тебя есть какой-либо вопрос или проблема - нажми на кнопку <b>Написать запрос</b> и мы в скором времени тебе ответим!', parse_mode='html', reply_markup=markup.markup_main())
    ...
    def send_text(message):
        ...
        if message.text == '✏️ Написать запрос':
            take_new_request = bot.send_message(message.chat.id, 'Введите свой запрос', reply_markup=markup.markup_cancel())
    ...
    def get_new_request(message):
        ...
        if check_file != None:
            ...
            if str(request) == 'None':
                ...
                for agent in agents:
                    try:
                        bot.send_message(agent[0], f'✅ Новый запрос под ID {req_id}')
                    except:
                        pass
    ...
    def get_additional_message(message, req_id, status):
        ...
        if additional_message == None:
            ...
            if status == 'user':
                ...
                agents = core.get_all_agents()
                for agent in agents:
                    bot.send_message(agent[0], f'✅ Новый ответ от пользователя на ID {req_id}')
        ...
        if call.message:
            ...
            #Открыть запрос
                ...
                for data in request_data:
                    ...
                    bot.send_message(chat_id=call.message.chat.id, text=data, parse_mode='Markdown', reply_markup=markup_req)
            ...
            #Завершить запрос
                ...
                if confirm_status == 'wait':
                    status_user = parts[3]
                    bot.send_message(chat_id=call.message.chat.id, text="Для завершения запроса - нажмите кнопку <b>Подтвердить</b>", parse_mode='html', reply_markup=markup.markup_confirm_req(req_id, status_user))
                ...
                #Подтвердить завершение
                    ...
                    if(status_user == "agent"):
                        user_id = core.get_user_id_of_req(req_id)
                        bot.send_message(chat_id=user_id, text=f"✅ Ваш запрос под ID {req_id} был закрыт агентом поддержки!")

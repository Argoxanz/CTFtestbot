...
def markup_request_action(req_id, req_status, callback):
    ...
    if req_status == 'confirm':
        ...
        item3 = types.InlineKeyboardButton("✅ Завершить запрос", callback_data=f'confirm_req:wait:{req_id}:{status_user}')
...
def markup_confirm_req(req_id, status_user):
    markup_confirm_req = types.InlineKeyboardMarkup(row_width=1)
    item1 = types.InlineKeyboardButton("✅ Подтвердить", callback_data=f'confirm_req:true:{req_id}:{status_user}')

...
def add_file(req_id, file_id, file_name, type):
    ...
    query = "INSERT INTO files (`req_id`, `file_id`, `file_name`, `type`) VALUES ('%s', '%s', '%s', '%s')"
    cur.execute(query, (req_id,file_id, file_name, type))
...
def new_req(user_id, request):
    ...
    query= "INSERT INTO messages (`req_id`, `message`, `user_status`, `date`) VALUES ('%s', '%s', 'user', '%s')"
    cur.execute(query, (req_id, request, date_now))
...
def add_message(req_id, message, user_status):
    ...
    #Добавить сообщение для запроса
    query = "INSERT INTO messages (`req_id`, `message`, `user_status`, `date`) VALUES ('%s', '%s', '%s', '%s')"
    cur.execute(query, (req_id, message, user_status, date_now))
...
def get_request_data(req_id, callback):
    ...
    cur.execute(f"SELECT `message`, `user_status`, `date`, `user_id` FROM messages AS m JOIN requests AS r ON r.req_id = m.req_id WHERE m.req_id = {req_id}")
    ...
    for message in messages:
        ...
        if user_status == 'user':
            if get_dialog_user_status == 'user':
                ...
                text_status = f'👤 Сообщение [пользователя](tg://user?id={user_id})'
        elif user_status == 'agent':
